...
export default function PricingCard({ plan, testIdPrefix = "pricing-card" }) {
  ...
  const labelCls = isHi ? "text-[#4FB8EE]" : "text-[#1E9BE0]";
  const cadenceCls = isHi ? "text-white/60" : "text-[#9CA3AF]";
  const bulletText = isHi ? "text-white/85" : "text-[#0A0A0A]";
  const bulletIcon = isHi ? "text-[#4FB8EE]" : "text-[#1E9BE0]";
  const ctaCls = isHi
    ? "bg-white text-[#0A0A0A] hover:bg-white/90"
    : "bg-[#1E9BE0] text-white hover:bg-[#1781BE]";
  ...
  return (
    ...
    >
      {tag && (
        <div className="absolute -top-3 left-7 px-2.5 py-1 rounded-full bg-[#1E9BE0] text-white text-[11px] font-mono tracking-wider uppercase">
          {tag}
        </div>
      )}
